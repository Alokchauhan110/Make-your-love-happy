...
HTML_TEMPLATE = """
<!-- The entire HTML template from before goes here. It is correct. -->
"""
HTML_TEMPLATE = """
...
GF_NAME, USER_NAME, INTRO, SONG, MEMORY1_PHOTO, MEMORY1_TITLE, MEMORY1_DESC, MEMORY2_PHOTO, MEMORY2_TITLE, MEMORY2_DESC, GALLERY_1, GALLERY_2, GALLERY_3, GALLERY_4, GALLERY_5, GALLERY_6, COUNTDOWN_OCCASION, COUNTDOWN_DATE, FINAL_HEADING, FINAL_MESSAGE = range(20)

# Define a dictionary to hold the questions and the next state
QUESTIONS = {
...
async def create(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    question, next_state_key = QUESTIONS[GF_NAME]
    context.user_data['current_state'] = GF_NAME
    await update.message.reply_text(question, parse_mode='Markdown')
    return 1 # A single state for handling replies

async def handle_reply(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    current_state = context.user_data.get('current_state')
    if current_state is None:
        return ConversationHandler.END # Should not happen, but a safeguard

    # Store the user's answer
    context.user_data['answers'][current_state] = update.message.text

    # Get the next question and state
    _question, next_state = QUESTIONS[current_state]

    if next_state is not None:
        # Ask the next question
        next_question_text, _ = QUESTIONS[next_state]
        context.user_data['current_state'] = next_state
        await update.message.reply_text(next_question_text, parse_mode='Markdown')
        return 1
    else:
        # All questions are answered, generate the file
...
def run_bot() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            1: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_reply)],
        },
